...
class sftpbast
{
    public static function upload($requestfile)
    {
        ...
        $disk             = Storage::disk('sftp'); //sftp
            ...
            $filePath     = env('SFTP_DIRECTORY') . '/tagihan_checklist/bast/' . $randomName .'.'. $file->getClientOriginalExtension(); // Menentukan jalur dan nama file tujuan di server FTP
    ...
    public static function reupload($filedelete, $requestfile)
    {
        ...
        $filePath = env('SFTP_DIRECTORY') . '/tagihan_checklist/bast/' . $filedelete;
    ...
    public static function showfile($requestfile)
    {
        ...
        $pdfPath =  env('SFTP_ROOT') .'/'. env('SFTP_DIRECTORY') .'/tagihan_checklist/bast/' . $requestfile;
    ...
    public static function hutangupload($requestfile)
    {
        ...
        // aktifkan write_enable=YES di file nano /etc/vsftpd.conf
        // Storage::disk('sftp')->delete('folder_path/file_name.jpg'); //delete file
        // return storage::disk('sftp')->download('/apk_keuanganv2/hutang/bast/'.$nama_bast); //download file
        $disk             = Storage::disk('sftp'); //sftp
            $file         = $requestfile;
            $randomName   = Str::random(40);
            $filePath     = env('SFTP_DIRECTORY') . '/hutang/bast/' . $randomName .'.'. $file->getClientOriginalExtension(); // Menentukan jalur dan nama file tujuan di server FTP
            $fileContents = file_get_contents($file->getRealPath()); // Membaca isi file menjadi string
            $nama_file    = $randomName .'.'. $file->getClientOriginalExtension();
        $disk->put($filePath, $fileContents);
    ...
    public static function hutangreupload($filedelete, $requestfile)
    {
        // Delete File Lama
        $disk     = Storage::disk('sftp');
        $filePath = env('SFTP_DIRECTORY') . '/hutang/bast/' . $filedelete;
        $disk->delete($filePath);
    ...
    public static function hutangshowfile($requestfile)
    {
        ...
        // Path file PDF di server NAS
        $pdfPath =  env('SFTP_ROOT') .'/'. env('SFTP_DIRECTORY') .'/hutang/bast/' . $requestfile;
