...
class sftpkuitansi
{
    public static function upload($requestfile)
    {
        ...
        $disk             = Storage::disk('sftp'); //sftp
            ...
            $filePath     = env('SFTP_DIRECTORY') . '/tagihan_checklist/kuitansi/' . $randomName .'.'. $file->getClientOriginalExtension(); // Menentukan jalur dan nama file tujuan di server FTP
    ...
    public static function reupload($filedelete, $requestfile)
    {
        ...
        $filePath = env('SFTP_DIRECTORY') . '/tagihan_checklist/kuitansi/' . $filedelete;
    ...
    public static function showfile($requestfile)
    {
        ...
        $pdfPath =  env('SFTP_ROOT') .'/'. env('SFTP_DIRECTORY') .'/tagihan_checklist/kuitansi/' . $requestfile;
    ...
    public static function hutangupload($requestfile)
    {
        ...
        $disk             = Storage::disk('sftp'); //sftp
            $file         = $requestfile;
            $randomName   = Str::random(40);
            $filePath     = env('SFTP_DIRECTORY') . '/hutang/kuitansi/' . $randomName .'.'. $file->getClientOriginalExtension(); // Menentukan jalur dan nama file tujuan di server FTP
            $fileContents = file_get_contents($file->getRealPath()); // Membaca isi file menjadi string
            $nama_file    = $randomName .'.'. $file->getClientOriginalExtension();
        $disk->put($filePath, $fileContents);
    ...
    public static function hutangreupload($filedelete, $requestfile)
    {
        // Delete File Lama
        $disk     = Storage::disk('sftp');
        $filePath = env('SFTP_DIRECTORY') . '/hutang/kuitansi/' . $filedelete;
        $disk->delete($filePath);
    ...
    public static function hutangshowfile($requestfile)
    {
        ...
        // Path file PDF di server NAS
        $pdfPath =  env('SFTP_ROOT') .'/'. env('SFTP_DIRECTORY') .'/hutang/kuitansi/' . $requestfile;
