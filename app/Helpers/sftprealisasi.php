...
class sftprealisasi
{
    public static function upload($requestfile)
    {
        ...
        $disk             = Storage::disk('sftp'); //sftp
            ...
            $filePath     = env('SFTP_DIRECTORY') . '/realisasi/checklist/' . $randomName .'.'. $file->getClientOriginalExtension(); // Menentukan jalur dan nama file tujuan di server FTP
    ...
    public static function reupload($filedelete, $requestfile)
    {
        ...
        $filePath = env('SFTP_DIRECTORY') . '/realisasi/checklist/' . $filedelete;
    ...
    public static function showfile($requestfile)
    {
        ...
        $pdfPath =  env('SFTP_ROOT') .'/'. env('SFTP_DIRECTORY') .'/realisasi/checklist/' . $requestfile;
    ...
    public static function uploadtagihan($requestfile)
    {
        // untuk menjalankan ftp ini, di server tujuan harus install apk vsftpd
        // aktifkan write_enable=YES di file nano /etc/vsftpd.conf
        // Storage::disk('sftp')->delete('folder_path/file_name.jpg'); //delete file
        ...
        $disk             = Storage::disk('sftp'); //sftp
            $file         = $requestfile;
            $randomName   = Str::random(40);
            $filePath     = env('SFTP_DIRECTORY') . '/realisasi/bendahara_pengeluaran/' . $randomName .'.'. $file->getClientOriginalExtension(); // Menentukan jalur dan nama file tujuan di server FTP
            $fileContents = file_get_contents($file->getRealPath()); // Membaca isi file menjadi string
            $nama_file    = $randomName .'.'. $file->getClientOriginalExtension();
        $disk->put($filePath, $fileContents);
    ...
    public static function reuploadtagihan($filedelete, $requestfile)
    {
        // Delete File Lama
        $disk     = Storage::disk('sftp');
        $filePath = env('SFTP_DIRECTORY') . '/realisasi/bendahara_pengeluaran/' . $filedelete;
        // $disk->exists($filePath)
        $disk->delete($filePath);
    ...
    public static function deletetagihan($filedelete)
    {
        $disk     = Storage::disk('sftp');
        $filePath = env('SFTP_DIRECTORY') . '/realisasi/bendahara_pengeluaran/' . $filedelete;
        return $disk->delete($filePath);
    ...
    public static function showfiletagihan($requestfile)
    {
        // Inisialisasi koneksi SFTP
        $sftp = new SFTP('192.168.1.128', 39615);
        if (!$sftp->login('apkKeuangan', 'aplikasiKeuangan123')) {
        ...
        // Path file PDF di server NAS
        $pdfPath =  env('SFTP_ROOT') .'/'. env('SFTP_DIRECTORY') .'/realisasi/bendahara_pengeluaran/' . $requestfile;
