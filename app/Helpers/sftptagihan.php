...
class sftptagihan
{
    public static function upload($requestfile)
    {
        ...
        $disk             = Storage::disk('sftp'); //sftp
            ...
            $filePath     = env('SFTP_DIRECTORY') . '/tagihan_spby/kuitansi/' . $randomName .'.'. $file->getClientOriginalExtension(); // Menentukan jalur dan nama file tujuan di server FTP
    ...
    public static function reupload($filedelete, $requestfile)
    {
        ...
        $filePath = env('SFTP_DIRECTORY') . '/tagihan_spby/kuitansi/' . $filedelete;
    ...
    public static function showfile($requestfile)
    {
        ...
        $pdfPath =  env('SFTP_ROOT') .'/'. env('SFTP_DIRECTORY') .'/tagihan_spby/kuitansi/' . $requestfile;
